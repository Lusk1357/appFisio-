...
exports.updatePatient = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, email, password, telefone, estado, cidade, bairro, endereco, cep, notes } = req.body;

        // Atualiza o nome/email/senha no User
        const updateData = {};
        ...
        // Atualiza campos extras no PatientProfile
        if (telefone !== undefined || estado !== undefined || cidade !== undefined || bairro !== undefined || endereco !== undefined || cep !== undefined || notes !== undefined) {
            await prisma.patientProfile.upsert({
                where: { userId: id },
                update: { telefone, estado, cidade, bairro, endereco, cep, notes },
                create: { userId: id, telefone, estado, cidade, bairro, endereco, cep, notes }
            });
        }
...
exports.deletePatient = async (req, res) => {
    try {
        ...
        // Apagar as tabelas filhas antes para não dar erro de restrição (Foreign Key)
        ...
        res.status(200).json({ sucesso: true, mensagem: "Paciente excluído com sucesso." });
    } catch (error) {
